...
public class MockConsumer<K, V> implements Consumer<K, V> {
    ...
    static final long DEFAULT_CLOSE_TIMEOUT_MS = 30 * 1000; // KafkaConsumer.DEFAULT_CLOSE_TIMEOUT_MS
     ...
     *
     * @param offsetResetStrategy offsetResetStrategy.
     */
    public MockConsumer(final OffsetResetStrategy offsetResetStrategy) {
     ...
     *
     * @param newAssignment newAssignment.
     */
    ...
    public synchronized void subscribe(final Pattern pattern, final ConsumerRebalanceListener listener) {
        ...
        for (final String topic: partitions.keySet()) {
    ...
    @Deprecated
    @Override
    public synchronized ConsumerRecords<K, V> poll(final long timeout) {
    ...
    public synchronized ConsumerRecords<K, V> poll(final Duration timeout) {
        ...
        synchronized (pollTasks) {
            final Runnable task = pollTasks.poll();
            if (task != null) {
                LOG.debug("poll: run poll task");
                task.run();
            }
        }
        ...
        if (pollException != null) {
            final RuntimeException exception = this.pollException;
            this.pollException = null;
            throw exception;
        }

        // Handle seeks that need to wait for a poll() call to be processed
        for (final TopicPartition tp : subscriptions.assignedPartitions()) {
        ...
        for (final Map.Entry<TopicPartition, List<ConsumerRecord<K, V>>> entry : this.records.entrySet()) {
            if (!subscriptions.isPaused(entry.getKey())) {
                final List<ConsumerRecord<K, V>> recs = entry.getValue();
                for (final ConsumerRecord<K, V> rec : recs) {
                    final long position = subscriptions.position(entry.getKey()).offset;
                    ...
                    if (assignment().contains(entry.getKey()) && rec.offset() >= position) {
                        ...
                        final Metadata.LeaderAndEpoch leaderAndEpoch = new Metadata.LeaderAndEpoch(Optional.empty(), rec.leaderEpoch());
                        final SubscriptionState.FetchPosition newPosition = new SubscriptionState.FetchPosition(
        ...
        LOG.debug("DONE with poll, results.size: {}", results.size());
     ...
     * @param record record.
     */
    public synchronized void addRecord(final ConsumerRecord<K, V> record) {
        ensureNotClosed();
        final TopicPartition tp = new TopicPartition(record.topic(), record.partition());
        final Set<TopicPartition> currentAssigned = this.subscriptions.assignedPartitions();
        if (!currentAssigned.contains(tp)) {
            throw new IllegalStateException("Cannot add records for a partition that is not assigned to the consumer");
        }
        final List<ConsumerRecord<K, V>> recs = this.records.computeIfAbsent(tp, k -> new ArrayList<>());
        recs.add(record);
    }
     ...
     */
    @Deprecated
    public synchronized void setException(final KafkaException exception) {
        setPollException(exception);
    }

    public synchronized void setPollException(final KafkaException exception) {
        this.pollException = exception;
    }

    public synchronized void setOffsetsException(final KafkaException exception) {
    ...
    @Override
    public synchronized void commitAsync(final OffsetCommitCallback callback) {
        ensureNotClosed();
    ...
    @Override
    public synchronized void commitSync(final Duration timeout) {
        commitSync(this.subscriptions.allConsumed());
    }
    ...
    @Override
    public synchronized long position(final TopicPartition partition, final Duration timeout) {
     ...
     */
    public synchronized void updatePartitions(final String topic, final List<PartitionInfo> partitions) {
        ensureNotClosed();
    ...
    @Override
    public synchronized void resume(final Collection<TopicPartition> partitions) {
        for (final TopicPartition partition : partitions) {
    ...
    public synchronized Map<TopicPartition, Long> beginningOffsets(final Collection<TopicPartition> partitions) {
        if (offsetsException != null) {
            final RuntimeException exception = this.offsetsException;
            this.offsetsException = null;
            throw exception;
        }
        final Map<TopicPartition, Long> result = new HashMap<>();
        for (final TopicPartition tp : partitions) {
            final Long beginningOffset = beginningOffsets.get(tp);
     ...
     */
    public synchronized void schedulePollTask(final Runnable task) {
        LOG.debug("schedulePollTask {}", task);
        synchronized (pollTasks) {
            pollTasks.add(task);
        }
    }
    ...
    private void updateFetchPosition(final TopicPartition tp) {
    ...
    private void resetOffsetPosition(final TopicPartition tp) {
        final OffsetResetStrategy strategy = subscriptions.resetStrategy(tp);
        final Long offset;
